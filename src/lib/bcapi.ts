...
export class BCJS{
  ...
  public readonly Host:string="https://localhost.bc-vault.com:1991/"
  ...
  public authTokenMatchPath: string = '/';
  ...
  public async getSupportedWalletTypes(device:number): Promise<WalletType[]>{
    ...
    if (typeof (newFormat[0]) === typeof(1)) {
      newFormat = (newFormat as number[]).map( x => this.fromLegacyWalletType(x) )
    }
  ...
  private fromLegacyWalletType(t:number):WalletType{
    let stringId: string|undefined;
    for(const legacyTypeProperty in WalletType_Legacy){
