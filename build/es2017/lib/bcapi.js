...
class BCJS {
            ...
            for (const deviceID of devArray) {
                ...
                try {
                    ...
                    if (e.BCHttpResponse !== undefined) {
                        const userData = await this.getWalletUserData(deviceID, types_1.WalletType.none, "", false);
                        devs.push({
                            id: deviceID,
    ...
    async getDevices() {
        let httpr;
        httpr = await this.getResponsePromised(types_1.Endpoint.Devices);
    ...
    async getFirmwareVersion(device) {
        let httpr;
        httpr = await this.getResponsePromised(types_1.Endpoint.FirmwareVersion, { device });
        this.assertIsBCHttpResponse(httpr);
        return httpr.body.data;
    }
    /**
    ...
    async getAvailableSpace(device) {
        let httpr;
        httpr = await this.getResponsePromised(types_1.Endpoint.AvailableSpace, { device });
        this.assertIsBCHttpResponse(httpr);
        return httpr.body.data;
    }
    /**
    ...
    /**
      ...
      @throws        Will throw an AxiosError if the request itself failed or if status code != 200
     */
    async EnterGlobalPin(device, passwordType = types_1.PasswordType.GlobalPassword) {
        const id = await this.getSecureWindowResponse(passwordType);
        this.log("Got pin popup:" + id);
        const httpr = await this.getResponsePromised(types_1.Endpoint.EnterGlobalPin, { device, password: id });
        this.assertIsBCHttpResponse(httpr);
    ...
    async web3_GetAccounts(cb) {
        try {
            ...
            try {
                const wallets = await this.getWalletsOfType(devices[0], types_1.WalletType.ethereum);
                cb(null, wallets.map((x) => "0x" + x));
            }
            catch (e) {
                if (e.BCHttpResponse !== undefined) {
                    // unlock BC Vault!
                    await this.EnterGlobalPin(devices[0], types_1.PasswordType.GlobalPassword);
                    const wallets = await this.getWalletsOfType(devices[0], types_1.WalletType.ethereum);
    ...
    async web3_signTransaction(txParams, cb) {
        try {
            ...
            const txHex = await this.GenerateTransaction(devices[devices.length - 1], types_1.WalletType.ethereum, txParams);
            cb(null, txHex);
        }
        catch (e) {
            cb(e, null);
    ...
    async web3_signPersonalMessage(msgParams, cb) {
        try {
            ...
            const signedMessage = await this.SignData(devices[devices.length - 1], types_1.WalletType.ethereum, msgParams.from, msgParams.data);
    ...
    async getVersion() {
        if (this.REMOTE_API_VERSION === undefined) {
            ...
            const response = await axios_1.default(this.getServerURL() + '/version');
    ...
    getResponsePromised(endpoint, data) {
        ...
        return new Promise(async (res, rej) => {
            if (this.endpointAllowsCredentials === undefined) {
                try {
                    const methodCheck = await axios_1.default({ baseURL: this.getServerURL(), data: "{}", method: "POST", url: "/Devices" });
    ...
    async getSecureWindowResponse(passwordType) {
        let preAuthObj;
        ...
        const x = await this.getResponsePromised(types_1.Endpoint.GetAuthID);
        const id = x.body;
